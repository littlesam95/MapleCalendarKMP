...
class AuthRepositoryImpl(
    ...
    override suspend fun autoLogin(apiKey: String): Flow<ApiState<CharacterBasic>> = flow {
        emit(ApiState.Loading)

        try {
            val response: AutoLoginResponse = dataSource.autoLogin(apiKey)

            // 데이터 계층의 모델을 도메인 모델로 변환
            val loginResult = response.toDomain()
            if (loginResult == null) {
                emit(ApiState.Error("캐릭터 정보를 불러올 수 없습니다."))
            } else {
                emit(ApiState.Success(loginResult))
            }
        } catch (e: Exception) {
